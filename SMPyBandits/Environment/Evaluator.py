            ...
            if self.useJoblib:
                ...
                for r in Parallel(n_jobs=self.cfg['n_jobs'], pre_dispatch='3*n_jobs', verbose=self.cfg['verbosity'])(
                        delayed(delayed_play)(env, policy, self.horizon, random_shuffle=self.random_shuffle,
                                              ...
                                              allrewards=allrewards, seed=seeds[repeatId], repeatId=repeatId,
                ...
                for repeatId in tqdm(range(self.repetitions), desc="Repeat"):
                    r = delayed_play(env, policy, self.horizon, random_shuffle=self.random_shuffle,
                                     random_invert=self.random_invert, nb_break_points=self.nb_break_points,
                                     allrewards=allrewards, repeatId=repeatId, useJoblib=self.useJoblib)
        ...
        if meanReward:
            ...
            if self.nb_break_points > 0:
                ...
                plt.ylabel(
                    ...
                    r"$\sum_{s=1}^{t} \sum_{k=1}^{%d} \mu_k(t) \mathbb{E}_{%d}[1(I(t)=k)]$" % (self.envs[envId].nbArms,
    ...
    for t in prettyRange:
        ...
        if env.isDynamic:
            if t in env.changePoints:
                ...
                if repeatId == 0: print(
                    "\nNew means vector = {}, best arm(s) = {}, at time t = {} ...".format(means, indexes_bestarm,
                                                                                           t))  # DEBUG
        ...
        if random_invert and t > 0 and t in t_events:
            ...
            result.change_in_arms(t, indexes_bestarm)
            if repeatId == 0: print(
                "\nInverting the order of the arms, best arm(s) = {}, at time t = {} ...".format(indexes_bestarm,
                                                                                                 t))  # DEBUG
