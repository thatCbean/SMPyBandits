...
class BOB(ContextualBasePolicy):
    """
    The BOB contextual adversarial bandit policy.
    """
    ...
    def __init__(self, nbArms, dimension, alpha=ALPHA,
        ...
        assert alpha > 0, "Error: the 'alpha' parameter for the BOB class must be greater than 0"
        assert beta > 0, "Error: the 'beta' parameter for the BOB class must be greater than 0"
        assert labda > 0, "Error: the 'lambda' parameter for the BOB class must be greater than 0"
    ...
    def __str__(self):
        return r"BOB($\alpha: {:.3g}, \beta: {:.3g}, \lambda: {:.3g}$)".format(self.alpha, self.beta, self.labda)
