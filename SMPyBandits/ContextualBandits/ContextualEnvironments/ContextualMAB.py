...
class ContextualMAB(object):
    ...
    def __init__(self, configuration):
        ...
        self.means = np.array(
            [arm.calculate_mean(self.context.get_means()) if isinstance(arm, ContextualArm) else arm.mean for arm in
             self.arms])  #: Means of arms
    ...
    def new_order_of_arm(self, arms):
        ...
        """
        assert sorted([arm.mean for arm in self.arms]) == sorted([arm.mean for arm in
                                                                  arms]), "Error: the new list of arms = {} does not have the same means as the previous ones."  # DEBUG
    ...
    def draw_with_context(self, armId, context, t=1):
        """ Return a random sample from the armId-th arm, at time t. """
        return self.arms[armId].draw(context, t)
    ...
    def plotHistogram(self, horizon=10000, savefig=None, bins=50, alpha=0.9, density=None):
        ...
        fig = plt.figure()
        for armId, arm in enumerate(arms):
            plt.hist(rewards[armId, :], bins=bins, density=density, color=colors[armId], label='$%s$' % repr(arm),
                     alpha=alpha)
        legend()
        plt.xlabel("Rewards")
        if density:
