...
class Plotting:
    ...
    def create_subfolder(self, N, environmentId, hashValue):
        subfolder = "{}/".format(datetime.datetime.now().strftime("%Y-%m-%d %H;%M;%S"))
    ...
    def plot_all_regrets(self, title, envId, semilogx, semilogy, loglog):
        if self.saveAllFigures:
            savefig = self.mainFigure
            print(" - Plotting the cumulative rewards, and saving the plot to {} ...".format(savefig))
            self.evaluator.plotRegrets(envId, title, savefig=savefig)
            ...
            self.evaluator.plotRegrets(envId, title)
            self.evaluator.plotRegrets(envId, title)
            self.evaluator.plotBestArmPulls(envId, title)
            self.evaluator.plotRegrets(envId, title, semilogy=True)
            if self.configuration['horizon'] >= 1000:
                self.evaluator.plotRegrets(envId, title, loglog=True)
            if self.configuration['repetitions'] > 1:
                # self.evaluator.plotRegrets(envId, semilogx=semilogx, semilogy=semilogy, loglog=loglog,
                #                            plotSTD=True) TODO: Fix this
                self.evaluator.plotRegrets(envId, title, semilogx=semilogx, semilogy=semilogy, loglog=loglog,
                                           plotMaxMin=True)
    ...
    def plot_all(self, envId, title=None):
        if not isinstance(self.evaluator, EvaluatorContextual):
        ...
        self.plot_all_regrets(title, envId, semilogx=SEMILOG_X, semilogy=SEMILOG_Y, loglog=LOG_LOG)
